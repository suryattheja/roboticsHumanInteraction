...
cmd->linear.x = 0.2; // cannot see obstacle or target at present, but seen obstacle and accumulated gcounter.
cmd->angular.z = 0.0; //Move in straight line after avoiding obstacle
cmdpub_.publish(cmd);
}
else{
geometry_msgs::TwistPtr cmd(new geometry_msgs::Twist());
...
{
std::cout << gcounter<< std::endl;// Spin until cannot see obstacle anymore and accumulate counter.
if(gcounter<50)
{
gcounter+=1;
}
